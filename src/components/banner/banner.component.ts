...
export class BannerComponent implements OnInit {
  ...
  getSingleProductDetail(id) {
    ...
    this.config.postHttp('getallproducts', dat).then((data: any) => {
      this.loading.hide();
      if (data.success == 1) {
        let product = data.product_data[0]
        console.log(product);
        this.shared.singleProductPageData.push(product);
        if (this.config.appNavigationTabs) {
          this.navCtrl.navigateForward("tabs/" + this.config.getCurrentHomePage() + "/product-detail/" + product.products_id);
        }
        else {
          this.navCtrl.navigateForward("product-detail/" + product.products_id);
        }
      }
    });
  }
